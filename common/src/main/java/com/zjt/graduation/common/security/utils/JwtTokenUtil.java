...
public class JwtTokenUtil {
    ...
    //@Value("${jwt.secret}")
    private String secret = "share-platform";

    private static String sec = "share-platform";
    ...
    private static Claims getClaims(String token) {
        ...
        try {
            ...
            LOGGER.info("JWT格式验证失败:{"+e.toString()+"}", token);
            throw new RuntimeException("token 格式验证失败");
        }
        return claims;
    }
     ...
     * 自定义生成token
     */
    public String myGeneratorToken(DefaultUserDetails userDetails) {
        Map<String, Object> claims = new HashMap<>();
        claims.put(CLAIM_KEY_USERNAME, userDetails.getUsername());
        claims.put(CLAIM_KEY_CREATED, new Date());
